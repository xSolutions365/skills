...
- Scope and user-visible outcomes: harden the `create-execplan` phase runner so each child phase stays a single fresh Codex process, cannot recurse into nested child-agent work, and still produces a validator-clean plan package through the normal workflow checkpoints.
...
- User-provided artifacts and starting views: `skills/create-execplan/SKILL.md`, the phase workflow references, `skills/create-execplan/scripts/run_phase.py`, `skills/create-execplan/scripts/run_codex_phase.sh`, `tests/run_create_execplan_helpers.sh`, and `tests/run_create_execplan_live_codex_smoke.sh`.
- Assumptions to validate with user: the child-phase recursion bug is reproducible from the repository's current `codex exec` integration and the parent controller may use a mock approval response to complete this end-to-end test scenario.
...
- Minimum smoke gate command: `CREATE_EXECPLAN_ENABLE_LIVE_CODEX=1 bash tests/run_create_execplan_live_codex_smoke.sh`
