...
4. Run a packet-executability review on every non-`Human` task row before asking for user review:
   - each `Code` task must name concrete `Edit Targets`
   - each `Read` task must name concrete `Supporting Context Anchors`
...
- Non-human task rows are explicit enough for a packet-only executor to act without repo-wide discovery.
