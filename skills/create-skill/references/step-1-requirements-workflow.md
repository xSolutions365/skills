...
2. Ask clarifying questions until each field is concrete and testable.
3. Require deterministic workflow boundaries:
   - contiguous step numbering
   - exactly one workflow reference file per step
   - no external path dependencies
4. Resolve tie-break behavior for any branching rule.
5. Request explicit user approval that requirements are frozen.
