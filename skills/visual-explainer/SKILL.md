...
- Write one self-contained HTML file under `.visual-explainer/` in the current project root.
- Report the chosen rendering mode, the output path, and any unresolved visual or tool-related limitations.
- Verify the result before handoff instead of returning an unchecked file.
